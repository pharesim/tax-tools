  ...
  for key, value in av.items():
    if key >= last:
      ...
      if av[tmp][0] >= 0:
        ...
        new_amount = av[tmp][0] * -1
        paid_off = amount - new_amount
        if paid_off > 0:
          old_paid = av[tmp][1]
          old_rate = ex[date][2]
          new_paid = old_paid + paid_off
          new_rate = (round((old_rate * old_paid)*100)/100 + round((paid_off * avg[tmp])*100)/100) / new_paid
          av[tmp][1] = new_paid
          ex[date][2] = new_rate
        
        av[tmp][0] = 0
        
        av, ex = spend(av, new_amount, rate, date, ex, tmp)
        av[tmp][0] = 0
        ex[date][0] += av[tmp][0]
        ex[date][2] = avg[tmp]
        return av, ex
...
#write to file
myfile = open('99_bitcoins_daily_report.csv','w')
...
for key, value in incomes.items():
  ...
  profit = (avg[key] - expenses[key][2]) * expenses[key][0] * -1
  print(key+" "+str(sum)+" "+str(value)+" "+str(avg[key])+" "+str(value*avg[key])+" "+str(total)+" "+str(expenses[key][0])+" "+str(available[key][2])+" "+str(expenses[key][1])+" "+str(profit)+" "+str(available[key][0]))
  wr.writerow([key, str(sum), str(value), str(avg[key]), str(value*avg[key]), str(total),  str(expenses[key][0]), str(round(available[key][2]*100000000)/100000000), str(expenses[key][1]), str(profit), str(available[key][0])])
myfile.close()
